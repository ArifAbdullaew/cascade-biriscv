...
//-------------------------------------------------------------
// Memory read/write (not connected yet)
//-------------------------------------------------------------
...
// Constructor
//-------------------------------------------------------------
riscv_tcm_top_rtl::riscv_tcm_top_rtl(sc_module_name name)
: sc_module(name)
{
    ...
    // Connect outputs
    m_axi_i_awvalid_out = m_rtl->axi_i_awvalid_o;
    m_axi_i_awaddr_out  = m_rtl->axi_i_awaddr_o;
    m_axi_i_wvalid_out  = m_rtl->axi_i_wvalid_o;
    m_axi_i_wdata_out   = m_rtl->axi_i_wdata_o;
    m_axi_i_wstrb_out   = m_rtl->axi_i_wstrb_o;
    m_axi_i_bready_out  = m_rtl->axi_i_bready_o;
    m_axi_i_arvalid_out = m_rtl->axi_i_arvalid_o;
    m_axi_i_araddr_out  = m_rtl->axi_i_araddr_o;
    m_axi_i_rready_out  = m_rtl->axi_i_rready_o;

    m_axi_t_awready_out = m_rtl->axi_t_awready_o;
    m_axi_t_wready_out  = m_rtl->axi_t_wready_o;
    m_axi_t_bvalid_out  = m_rtl->axi_t_bvalid_o;
    m_axi_t_bresp_out   = m_rtl->axi_t_bresp_o;
    m_axi_t_bid_out     = m_rtl->axi_t_bid_o;
    m_axi_t_arready_out = m_rtl->axi_t_arready_o;
    m_axi_t_rvalid_out  = m_rtl->axi_t_rvalid_o;
    m_axi_t_rdata_out   = m_rtl->axi_t_rdata_o;
    m_axi_t_rresp_out   = m_rtl->axi_t_rresp_o;
    m_axi_t_rid_out     = m_rtl->axi_t_rid_o;
    m_axi_t_rlast_out   = m_rtl->axi_t_rlast_o;

    // Setup SystemC process
    SC_METHOD(async_outputs);
    sensitive << clk_in << rst_in << rst_cpu_in << intr_in;
    sensitive << axi_i_in << axi_t_in;
#if VM_TRACE
    SC_METHOD(trace_rtl);
    sensitive << clk_in;
    m_vcd = nullptr;
...
void riscv_tcm_top_rtl::trace_rtl(void)
{
    ...
    if (m_delay_waves) {
        if (sc_time_stamp() > m_waves_start) {
            std::cout << "WAVES: Delayed start reached - " << sc_time_stamp() << std::endl;
            m_delay_waves = false;
        }
    }
    else if (m_vcd) {
